...
class DeblurGANv2(nn.Module):
    ...
    def forward(self, x):
        ...
        map4 = self.fpnup1(x4 + F.interpolate(x5, scale_factor = 2, mode = 'nearest'))         # out: batch * 128 * 16 * 16
        map3 = self.fpnup2(x3 + F.interpolate(map4, scale_factor = 2, mode = 'nearest'))       # out: batch * 128 * 32 * 32
        map2 = self.fpnup3(x2 + F.interpolate(map3, scale_factor = 2, mode = 'nearest'))       # out: batch * 128 * 64 * 64
        #map1 = x1                                                                          # out: batch * 128 * 128 * 128
        # Final interpolate
        f5 = F.interpolate(self.head1(x5), scale_factor = 8, mode = 'nearest')                 # out: batch * 64 * 64 * 64
        f4 = F.interpolate(self.head2(map4), scale_factor = 4, mode = 'nearest')               # out: batch * 64 * 64 * 64
        f3 = F.interpolate(self.head3(map3), scale_factor = 2, mode = 'nearest')               # out: batch * 64 * 64 * 64
        f2 = self.head4(map2)                                                               # out: batch * 64 * 64 * 64
        fusion = torch.cat((f2, f3, f4, f5), 1)                                             # out: batch * 256 * 64 * 64
        fusion = F.interpolate(self.fusion1(fusion), scale_factor = 2, mode = 'nearest')       # out: batch * 128 * 128 * 128
        fusion = F.interpolate(self.fusion2(fusion + x1), scale_factor = 2, mode = 'nearest')  # out: batch * 64 * 256 * 256
